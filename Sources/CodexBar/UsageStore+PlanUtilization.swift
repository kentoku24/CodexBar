...
extension UsageStore {
    ...
    func planUtilizationHistory(for provider: UsageProvider) -> [PlanUtilizationSeriesHistory] {
        if self.isLocalFileSourceActive(for: provider) {
            return []
        }
    ...
    func shouldHidePlanUtilizationMenuItem(for provider: UsageProvider) -> Bool {
        ...
        if self.isLocalFileSourceActive(for: provider) { return true }
        ...
        async
    {
        ...
        guard !self.isLocalFileSourceActive(for: provider) else { return }
