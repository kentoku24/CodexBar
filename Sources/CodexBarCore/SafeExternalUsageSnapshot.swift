...
public struct SafeExternalUsageSnapshot: Codable, Equatable, Sendable {
    public let providers: [SafeExternalProviderSnapshot]

    public init(providers: [SafeExternalProviderSnapshot]) {
        self.providers = providers.sorted { $0.provider.rawValue < $1.provider.rawValue }
    }

    public func providerSnapshot(for provider: UsageProvider) -> SafeExternalProviderSnapshot? {
        self.providers.first { $0.provider == provider }
    }
}

public struct SafeExternalProviderSnapshot: Codable, Equatable, Sendable {
...
public enum SafeExternalUsageSnapshotStore {
    ...
    public static func load(from url: URL) throws -> SafeExternalUsageSnapshot? {
        guard FileManager.default.fileExists(atPath: url.path) else { return nil }
        let data = try Data(contentsOf: url)
        return try self.decoder.decode(SafeExternalUsageSnapshot.self, from: data)
    }

    public static func load(env: [String: String] = ProcessInfo.processInfo
        .environment) throws -> SafeExternalUsageSnapshot?
    {
        guard let url = self.defaultFileURL(env: env) else { return nil }
        return try self.load(from: url)
    }

    public static func save(_ snapshot: SafeExternalUsageSnapshot, to url: URL) throws {
